defmodule Spex do
  ...
  _labelled transition system_ (see the [Theoretical background section of the readme]\
  (README.md#theoretical-background)).

  Optionally, add `:spex` to your [`.formatter.exs` file under `:import_deps`]\
  (https://hexdocs.pm/mix/main/Mix.Tasks.Format.html#module-importing-dependencies-configuration).
  ...
  Alternatively, you can also add either instance manager to your supervision tree directly, e.g.:
      ...
      children = [
        ...
        {Spex.InstanceManager.DistributedInstanceManager, [distribution_factor: 4] ++ \
  spex_config_opts}
      ]
  ...
  For (1), adjust/add an error handler in your specifications, e.g. as so:
      ...
      defmodule YourApp.Specifications.Tree do
        ...
        def error_handler(%Spex.Errors.ImplModelError{reason: :impl_model_not_found}, _caller), \
  do: :ok
  ...
  def child_spec(opts) do
    opts = Keyword.merge(@instance_manager_opts, opts)
    @instance_manager.child_spec(opts)
  end
